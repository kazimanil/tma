...
happiness <- rbind(
    data_2003[, .(
        happiness = scale_transformation(s8, minimum = 5, maximum = 1),
        weights = ff,
        year = 2003
    )],
    data_2004[, .(
        happiness = scale_transformation(b07, minimum = 5, maximum = 1),
        weights = ff,
        year = 2004
    )],
    data_2005[, .(
        happiness = scale_transformation(bsoru7, minimum = 5, maximum = 1),
        weights = ff,
        year = 2005
    )],
    data_2006[, .(
        happiness = scale_transformation(bs7, minimum = 5, maximum = 1),
        weights = faktor_fert,
        year = 2006
    )],
    data_2007[, .(
    ...
    data_2009[, .(
        happiness = scale_transformation(b9, minimum = 5, maximum = 1),
        weights = ff,
        year = 2009
    )],
    data_2010[, .(
    ...
    data_2012[, .(
        happiness = scale_transformation(b08, minimum = 5, maximum = 1),
        weights = ff,
        year = 2012
    )],
    data_2013[, .(
    ...
    data_2015[, .(
        happiness = scale_transformation(MUTLULUK, minimum = 5, maximum = 1),
...
comparison <- data.table(
    ...
    happiness = happiness[
        !is.na(happiness), .(avg_happiness = sum(happiness * weights) / sum(weights)),
        .(year)
    ]$avg_happiness,
    # source : World Bank
    # https://data.worldbank.org/indicator/NY.GDP.PCAP.CD?end=2024&locations=TR&start=2003&view=chart
    gdp_pc = c(
        4637.9, 5960.8, 7303, 7953.1, 9711.2, 10843.5, 9013, 10622.7, 11300.8, 11713.3,
        12578.2, 12165.2, 11050, 10970, 10695.6
    )
)

rm(happiness)
fwrite(comparison, "agg_data/happiness_gdp_turkey_2003_2017.csv")
